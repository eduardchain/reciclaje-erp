        ...
        <div className="space-y-4">
          <Card>
            <CardContent className="pt-6">
              <p className="text-sm text-gray-500">Margen Global</p>
              <p className="text-3xl font-bold">{(data.overall_margin * 100).toFixed(1)}%</p>
            </CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle className="text-base">Analisis por Material</CardTitle></CardHeader>
            <CardContent>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right">Compra Qty</TableHead>
                      <TableHead className="text-right">Compra $</TableHead>
                      <TableHead className="text-right">Precio Compra</TableHead>
                      <TableHead className="text-right">Venta Qty</TableHead>
                      <TableHead className="text-right">Venta $</TableHead>
                      <TableHead className="text-right">Precio Venta</TableHead>
                      <TableHead className="text-right">Utilidad</TableHead>
                      <TableHead className="text-right">Margen</TableHead>
                      <TableHead className="text-right">DE Qty</TableHead>
                      <TableHead className="text-right">DE Util.</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {data.materials.map((m) => (
                      <TableRow key={m.material_id}>
                        ...
                        <TableCell className={`text-right font-medium ${m.gross_profit >= 0 ? "text-green-700" : "text-red-700"}`}>{formatCurrency(m.gross_profit)}</TableCell>
                        ...
                        <TableCell className="text-right text-green-700">{formatCurrency(m.double_entry_profit)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
