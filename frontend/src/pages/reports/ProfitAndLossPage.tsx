        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Separator />
              <div className="flex justify-between font-medium"><span>Utilidad Bruta Ventas</span><span className={data.gross_profit_sales >= 0 ? "text-green-700" : "text-red-700"}>{formatCurrency(data.gross_profit_sales)} ({(data.gross_margin_sales * 100).toFixed(1)}%)</span></div>
            </div>

            <div className="space-y-2">
              <div className="flex justify-between text-sm"><span>Utilidad Pasa Mano ({data.double_entry_count} operaciones)</span><span className="font-medium text-green-700">{formatCurrency(data.double_entry_profit)}</span></div>
              <Separator />
              <div className="flex justify-between font-medium"><span>Utilidad Bruta Total</span><span className={data.total_gross_profit >= 0 ? "text-green-700" : "text-red-700"}>{formatCurrency(data.total_gross_profit)}</span></div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Separator />
              <div className="flex justify-between text-lg font-bold"><span>Utilidad Neta</span><span className={data.net_profit >= 0 ? "text-green-700" : "text-red-700"}>{formatCurrency(data.net_profit)} ({(data.net_margin * 100).toFixed(1)}%)</span></div>
            </div>
            ...
            {data.expenses_by_category.length > 0 && (
              <>
                <Separator />
                <div>
                  <p className="font-medium mb-2">Desglose de Gastos</p>
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {data.expenses_by_category.map((cat) => (
                        <TableRow key={cat.category_id ?? "none"}>
                          ...
                          <TableCell className="text-sm text-gray-500">{cat.is_direct_expense ? "Directo" : "Indirecto"}</TableCell>
