...
function getColumns(canViewProfit: boolean): ColumnDef<DoubleEntryResponse, unknown>[] {
  return [
    { accessorKey: "double_entry_number", header: "#", cell: ({ row }) => <span className="font-medium">#{row.original.double_entry_number}</span>, meta: { hideOnMobile: true } },
    { accessorKey: "invoice_number", header: "FACTURA", cell: ({ row }) => row.original.invoice_number || "—", meta: { hideOnMobile: true } },
    ...
    { accessorKey: "materials_summary", header: "DETALLE", cell: ({ row }) => <span className="text-xs text-slate-600">{row.original.materials_summary}</span>, meta: { hideOnMobile: true } },
    ...
    ...(canViewProfit ? [
      { accessorKey: "profit", header: "UTILIDAD BRUTA", cell: ({ row }: { row: { original: DoubleEntryResponse } }) => <span className={`font-medium tabular-nums ${row.original.profit >= 0 ? "text-emerald-700" : "text-red-700"}`}>{formatCurrency(row.original.profit)}</span>, meta: { hideOnMobile: true } } as ColumnDef<DoubleEntryResponse, unknown>,
