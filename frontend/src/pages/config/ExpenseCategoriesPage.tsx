...
const columns: ColumnDef<ExpenseCategoryResponse, unknown>[] = [
  ...
  { accessorKey: "is_direct_expense", header: "Tipo", cell: ({ row }) => row.original.is_direct_expense ? <Badge variant="outline" className="bg-blue-50 text-blue-700">Directo</Badge> : <Badge variant="outline" className="bg-gray-50 text-gray-700">Indirecto</Badge> },
];
  ...
  return (
    <ConfigLayout>
      <div className="flex justify-end">
        <Button onClick={() => openDialog(null)} className="bg-green-600 hover:bg-green-700"><Plus className="h-4 w-4 mr-2" />Nueva Categoria</Button>
      </div>

      <DataTable columns={columns} data={data?.items ?? []} loading={isLoading} pageCount={1} pageIndex={0} pageSize={100} onPageChange={() => {}}
        onRowClick={(row) => openDialog(row)} emptyTitle="Sin categorias" emptyDescription="No hay categorias de gasto." />
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm">Gasto Directo (afecta costo material)</span>
