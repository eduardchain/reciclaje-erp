    ...
    ...(showReceiving
      ? [{
          ...
          cell: ({ row }: { row: { original: WarehouseResponse } }) =>
            row.original.is_transit ? (
              <span className="bg-indigo-100 text-indigo-700 text-xs px-1.5 py-0.5 rounded font-medium">Tránsito</span>
...
export default function WarehousesPage() {
  ...
  // E3.1: campos de transito solo con two_step (runbook: crear JM-TRANSITO aqui)
  const showTransit = flagEnabled("two_step_transfers_enabled");
              ...
              <div className="space-y-2">
                <div className="flex items-start gap-2">
                  <Checkbox
                    id="wh-transit"
                    checked={isTransit}
                    onCheckedChange={(v) => setIsTransit(v === true)}
                    className="mt-0.5"
                  />
                  <div>
                    <Label htmlFor="wh-transit" className="text-sm font-normal cursor-pointer">
                ...
                {isTransit && (
                  <Select value={transitTarget} onValueChange={setTransitTarget}>
                    ...
                    <SelectContent>
                      {(data?.items ?? [])
                        .filter((w) => w.is_active && !w.is_transit && w.id !== editItem?.id)
                        .map((w) => (
